...
// Evita cache agressivo que impede atualizacao imediata apos uploads/edicoes
...
try {
    ...
    $subpastas_count = count($subpastas);
    ...
    $docs_count_page = count($documentos);

    // Highlighter simples para o termo buscado (título/descrição)
    $highlight = function($text) use ($q) {
        $text = (string)$text;
        $safe = htmlspecialchars($text, ENT_QUOTES, 'UTF-8');
        if ($q === '') { return $safe; }
        $pattern = '/' . preg_quote($q, '/') . '/i';
        return preg_replace($pattern, '<mark class="doc-highlight">$0</mark>', $safe);
    };
...
<div class="content-wrapper">
    ...
    <section class="content-header">
            ...
            <div class="row align-items-center">
                <div class="col-lg-7 col-md-6">
                    <div class="doc-hero-title">
                        <span class="eyebrow">Espaço de Documentos</span>
                        <h1><?= $titulo_pagina ?></h1>
                        <p class="subtitle">Visual moderno inspirado no eDok: filtros rápidos, grade/lista e ações em lote.</p>
                    </div>
                </div>
                <div class="col-lg-5 col-md-6">
                    <div class="doc-hero-metrics">
                        <div class="metric">
                            <div class="metric-label">Nesta pasta</div>
                            <div class="metric-value"><?= $total_encontrados ?></div>
                            <div class="metric-sub">Itens totais</div>
                        </div>
                        <div class="metric">
                            <div class="metric-label">Subpastas</div>
                            <div class="metric-value"><?= $subpastas_count ?></div>
                            <div class="metric-sub">Organização local</div>
                        </div>
                        <div class="metric">
                            <div class="metric-label">Página</div>
                            <div class="metric-value"><?= $docs_count_page ?></div>
                            <div class="metric-sub">Exibidos agora</div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row mb-2"><div class="col-sm-12">
                <ol class="breadcrumb float-sm-left doc-breadcrumb-modern">
        ...
        <div class="container-fluid">
            <div class="card card-outline documentos-card">
                <div class="card-header">
                    <div class="doc-toolbar d-flex flex-column flex-xl-row justify-content-between align-items-start">
                        <div class="doc-toolbar-left d-flex align-items-center flex-wrap mb-2 mb-xl-0">
                            <form method="get" class="form-inline mr-3">
                                <?php if ($pasta_atual_id): ?>
                                    <input type="hidden" name="pasta_id" value="<?= htmlspecialchars($pasta_atual_id) ?>">
                                <?php endif; ?>
                                ...
                                <div class="input-group input-group-sm ml-2">
                                    <input type="text" class="form-control" name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Digite sua pesquisa e tecle enter...">
                                    <div class="input-group-append"><button class="btn btn-default" type="submit"><i class="fas fa-search"></i></button></div>
                                </div>
                                <?php if ($tipo_id): ?>
                                    <input type="hidden" name="tipo_id" value="<?= (int)$tipo_id ?>">
                                <?php endif; ?>
                            </form>
                            <div class="view-toggle btn-group btn-group-sm mr-3">
                                <a href="<?= htmlspecialchars($linkList) ?>" id="btn-list-view" class="btn <?= $view === 'list' ? 'btn-primary active' : 'btn-outline-secondary' ?>" title="Lista"><i class="fas fa-th-list"></i></a>
                                <a href="<?= htmlspecialchars($linkGrid) ?>" id="btn-grid-view" class="btn <?= $view === 'grid' ? 'btn-primary active' : 'btn-outline-secondary' ?>" title="Grade"><i class="fas fa-th-large"></i></a>
                            </div>
                            <span class="doc-count text-muted small">Mostrando de <span class="count-badge"><?= $offset + 1 ?></span> a <span class="count-badge"><?= $offset + count($documentos) ?></span> de <span class="count-badge"><?= $total_encontrados ?></span></span>
                            <?php if ($q !== ''): ?>
                                <span class="badge badge-amber ml-2">Resultado para "<?= htmlspecialchars($q) ?>"</span>
                            <?php endif; ?>
                        </div>
                        <div class="doc-toolbar-right d-flex align-items-center flex-wrap">
                            <a href="pastas_criar.php<?= $pasta_atual_id ? ('?pasta_pai_id='.(int)$pasta_atual_id) : '' ?>" class="btn btn-sm btn-secondary mb-2 mb-sm-0"><i class="fas fa-folder-plus"></i> Criar Pasta</a>
                            <a href="pastas_criar_lote.php<?= $pasta_atual_id ? ('?pasta_pai_id='.(int)$pasta_atual_id) : '' ?>" class="btn btn-sm btn-outline-secondary ml-2 mb-2 mb-sm-0"><i class="fas fa-layer-group"></i> Criar Pastas (Lote)</a>
                            <?php if ($pasta_atual_id): ?>
                                <a href="documentos_adicionar.php?pasta_id=<?= (int)$pasta_atual_id ?>" class="btn btn-sm btn-success ml-2 mb-2 mb-sm-0"><i class="fas fa-plus"></i> Criar Documento</a>
                            <?php endif; ?>
                            <a href="digitalizar.php" class="btn btn-sm btn-outline-primary ml-2" title="Digitalizar ou Upload em lote"><i class="fas fa-file-upload"></i> Digitalizar</a>
                        </div>
                    </div>
                </div>
                <div class="doc-filters px-3 pt-3 pb-3 border-top">
                    <div class="doc-chips d-flex align-items-center flex-wrap mb-2">
                    ...
                    <div class="doc-chips d-flex align-items-center flex-wrap mt-2">
                ...
                <div class="items-container">
                    ...
                    <?php foreach($documentos as $doc): ?>
                        <div class="item" role="button" onclick="window.location.href='documentos_ver?id=<?= htmlspecialchars($doc['id']); ?>'" title="Abrir: <?= htmlspecialchars($doc['titulo']); ?>">
                            ...
                            <div class="item-name"><?= $highlight($doc['titulo']); ?></div>
                            <?php if (!empty($doc['descricao'])): ?>
                                <div class="item-details text-muted small"><?= $highlight($doc['descricao']); ?></div>
                            <?php endif; ?>
            ...
            <thead>
                <tr>
                    ...
                    <th style="width:20%" class="text-right">Acoes</th>
                </tr>
            </thead>
            <tbody>
                ...
                <?php foreach($documentos as $doc):?><?php $vparam = !empty($doc['atualizado_em']) ? strtotime($doc['atualizado_em']) : time(); $ext = strtolower(pathinfo($doc['caminho_arquivo'] ?? '', PATHINFO_EXTENSION)); $extClass = in_array($ext, ['pdf','doc','docx','xls','xlsx','jpg','jpeg','png']) ? $ext : 'outro'; ?><tr class="doc-row">
                    <td><input type="checkbox" class="check-item" value="d-<?= htmlspecialchars($doc['id']);?>"></td>
                    <td><?= htmlspecialchars($doc['id']) ?></td>
                    <td class="title-cell"><a href="documentos_ver?id=<?= htmlspecialchars($doc['id']);?>&v=<?= $vparam; ?>" title="Visualizar"><?php if(!empty($ext)): ?><span class="ext-badge <?= $extClass ?>"><?= strtoupper($ext) ?></span><?php endif; ?><i class="fas fa-file-alt mr-2 text-muted"></i><strong><?= $highlight($doc['titulo'] ?? ''); ?></strong></a><?php if (!empty($doc['descricao'])): ?><br><small class="ml-4"><?= $highlight($doc['descricao'] ?? ''); ?></small><?php endif; ?></td>
                    ...
                    <td class="updated-cell"><?= !empty($doc['atualizado_em']) ? date('d/m/Y H:i',strtotime($doc['atualizado_em'])) : (!empty($doc['data_upload']) ? date('d/m/Y H:i',strtotime($doc['data_upload'])) : '-') ?></td>
                    <td class="text-right actions-cell">
                        ...
                        <a href="esign/solicitar.php?id=<?= htmlspecialchars($doc['id']);?>" class="action-icon text-primary" title="Solicitar Assinatura por E-mail"><i class="fas fa-envelope"></i></a>
                        <a href="itens_mover.php?ids=<?= urlencode('d-' . (int)$doc['id']); ?>" class="action-icon text-secondary" title="Mover"><i class="fas fa-arrows-alt"></i></a>
...
$(function(){
  ...
  try{
    ...
    function toggleActionBar(){
        ...
        if(any) $('#card-footer-actions').slideDown('fast').removeClass('d-none'); else $('#card-footer-actions').slideUp('fast');
